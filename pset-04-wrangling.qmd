...
```{r}
population |> filter(state %in% c("DC", "PR")) |> kable()
```

---

**Task 7: As a check, make a barplot of states' 2020 and 2021 populations. Show the state names in the y-axis ordered by population size.  Hint: You will need to use `reorder` and use `facet_wrap`.**
...
```
The above code shows the time range of the data.
